function getHardhatRuntimeEnvironment() {
  ...
  async function main() {
    ...
    const targetFileAbsolutePath = `${hre.config.paths.sources}/pools/weighted/WeightedPoolFactory.sol`;
...
//   `
    ...
    const a = await hre.run(TASK_FLATTEN_GET_FLATTENED_SOURCE, {
        files: Object.keys(output.contracts)
    });
    console.log(a)
