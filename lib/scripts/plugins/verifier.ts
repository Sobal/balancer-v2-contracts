...
export default class Verifier {
  ...
  async getVerificationParams(name: string, address: string, args: string | unknown[]): Promise<any> {
    ...
    const sources = Object.keys(build.sources).map(
      name => [
        name,
  ...
  getBuildInfo(fileName: string): BuildInfo {
    const buildInfoDir = this._dirAt(config.paths.root, 'artifacts/build-info');
  ...
  private async attemptVerification(
    ...
    const sources = Object.keys(compilerInput.sources).map(
      name => {
        return {
